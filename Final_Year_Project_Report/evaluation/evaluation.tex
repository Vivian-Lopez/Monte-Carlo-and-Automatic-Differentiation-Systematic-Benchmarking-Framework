...
old-profiler versus SHA selection quality. GPU acceleration, distributed
execution, broad BLAS-environment sweeps, mixed precision, and Mojo
vendor-claim validation are not reported as completed experiments in this
chapter.
...
\(T=1\), the Black--Scholes analytical call price is \(10.4506\), with
...
Rust/Rayon is the fastest no-AD engine across the full path-count sweep.  At
...
Rust remains the fastest engine in this experiment, followed by C++ and JAX.
...
implementation is the strongest no-AD local-volatility engine in the current
local environment.
...
The stress test reinforces two conclusions.  First, Rust remains faster than
...
cost-performance rows for priced cloud instances at \(M=100{,}000\). It
reports both runtime and estimated cost per run, making explicit that a faster
machine can also be cheaper per run if the runtime improvement outweighs the
hourly price difference.

\input{evaluation/tables/cloud_cost_performance.tex}

Figure~\ref{fig:cloud-cost-performance} visualises the same relationship. In
the stored priced comparison, \code{t2d-standard-4} is faster and lower cost
per run than \code{n2-standard-4} for the listed best no-AD workloads. This is
an empirical result for the measured workloads and pricing assumptions, not a
claim about all machine types or future cloud prices.
...
This result directly addresses the cloud resource-selection goal of the
project. In the measured grid, SHA halves the number of full-scale cloud
benchmark cells while preserving the full-grid decision quality on the
reported metrics. The practical significance is that the expensive part of the
experiment is not merely shifted elsewhere: the SHA probe path budget is
95.5\% lower than evaluating the grid at full scale, and the mean scaling-law
extrapolation error is 9.7\%.

\subsection{SHA Progression}
\label{subsec:sha-progression}

Figure~\ref{fig:sha-progression} shows the SHA promotion sequence recorded for
the final experiment. All 16 configurations are evaluated at the cheapest
probe level, then the active set is reduced as the path budget increases. The
final selected set contains the configurations promoted for full benchmarking
after the profiler's conservative safeguards are applied.
...
on throughput.  Rust/Rayon is the fastest no-AD engine across the main
...
\subsection{Programming-Language and AD Trade-offs}
\label{subsec:discussion-language-ad}

The compiled CPU engines provide the strongest no-AD local-volatility
performance.  Rust/Rayon performs best overall in the local-volatility
experiments, while C++/OpenMP remains competitive and scales well with
...
\subsection{AI/ML Compiler Techniques}
\label{subsec:discussion-ai-techniques}

The JAX/XLA results provide partial evidence for the relevance of AI/ML
compiler techniques to Monte Carlo simulation.  JAX is relatively weak on the
...
structure to amortise dispatch and compilation overheads.

However, the results do not test mixed precision, kernel auto-tuning, GPU
execution, or hardware-aware scheduling beyond JAX/XLA's CPU backend.
Therefore, the AI/ML-technique conclusion should be limited to compiled XLA
execution and automatic differentiation support, not to the full range of
AI-system optimisation techniques.

\subsection{Scope Against the Original Proposal}
...
profiler-selection evidence. GPU acceleration, distributed execution, broad
BLAS-environment comparison, and Mojo vendor-claim validation remain valid
extensions of the framework but are outside the completed result set.
...
convergence with larger \(M\), rather than by exact analytical error.  Price
differences between engines may also reflect different random-number
generators or path-generation implementations.
...
\subsection{Excluded Proposal Extensions}
\label{subsec:limitations-excluded-extensions}

The original project proposal included cloud resource selection, GPU
acceleration, distributed execution, mixed precision, BLAS backend sweeps, and
Mojo vendor-claim validation. Cloud resource selection is partially completed
through the final cost-performance and profiler results. GPU acceleration,
distributed execution, mixed precision, wider BLAS sweeps, and Mojo
vendor-claim validation are future extensions that the framework is designed
to support, but they are not part of the completed experimental evidence.
...
\begin{itemize}
    \item \textbf{European GBM validation:} the simple GBM workload verifies
    that the framework produces credible prices and AD Greeks against
    Black--Scholes references.  It is useful as a validation workload, but
    not representative enough to be the main benchmark.

    \item \textbf{Local-volatility engine performance:} Rust/Rayon is the
    fastest no-AD local-volatility engine across the main \(M\)-sweep and
    the large stress test.  JAX/XLA and C++/OpenMP are close at larger
    local-volatility sizes, while NumPy is consistently slower.

    \item \textbf{AD overhead:} reverse-mode AD is substantially cheaper
    than forward-mode AD for the scalar-output local-volatility pricing
    workload.  Reverse-mode overhead is usually around \(2.1\)--\(2.6\times\),
    while forward-mode overhead is around \(4\)--\(7\times\).

    \item \textbf{Time-step scaling:} runtime grows approximately linearly
    with the number of local-volatility time steps \(N\), confirming that
    the workload cost is dominated by repeated scan updates.

    \item \textbf{Memory limits:} JAX reverse-mode AD reaches practical
    memory limits at large \(M,N\) combinations, including
    \(M=250k,N=504\) and \(M=1M,N=252\).

    \item \textbf{Thread scaling:} C++ and Rust scale effectively with
    requested CPU threads, while JAX does not respond meaningfully to the
    same thread-count controls because XLA manages CPU execution internally.

    \item \textbf{Measurement stability:} key benchmark cells have CV values
    between 3.23\% and 6.86\%, which is acceptable for interpreting the main
    performance differences observed in this chapter.

    \item \textbf{Cloud cost-performance:} the final priced cloud comparison
    shows that instance choice can change both runtime and estimated per-run
    cost. In the stored comparison, \code{t2d-standard-4} is faster and lower
    cost per run than \code{n2-standard-4} for the listed best no-AD rows.

    \item \textbf{Budget-aware profiling:} SHA selects 24 of 48 full-grid
    benchmark cells, saving 50.0\% of full runs while preserving 100.0\%
    Pareto recovery and 0.0\% runtime and cost regret in the final measured
    cloud grid.
\end{itemize}

Overall, the results show that MC+AD performance depends strongly on the
interaction between workload structure, runtime system, differentiation
mode, parallel execution model, and cloud cost assumptions. The
local-volatility suite demonstrates that the framework can expose detailed
runtime, AD, memory, and scaling trade-offs in a reproducible way, while the
final cloud-profiler experiment demonstrates that cheap probes and SHA can
reduce full cloud benchmarking work without losing decision quality on the
measured grid.
