...
\input{evaluation/tables/guarded_sha_localvol.tex}
...
RNG-stream issue that requires further investigation.
...
consistent JAX AD outputs, not as analytically validated Greeks. A
finite-difference check using common random numbers would be the most useful
...
\begin{itemize}
    ...
    \item \textbf{Budget-aware profiling:} plain SHA halves the full-grid work
    on the favourable \code{n2-standard-4} cloud slice, but the guarded
    local-volatility profiler gives the stronger final result. Across five
    four-vCPU instance types, guarded SHA matches the observed objective
    oracle while avoiding 13 of 30 full-scale task-level runs. Rust remains the
    best all-backend price-only performance candidate, but the validated
    price-only deployment recommendation excludes Rust and selects
    \code{cpp/none/t2d-standard-4}.
\end{itemize}
