...
\input{evaluation/tables/localvol_uncertainty.tex}
...
Rust/Rayon is the fastest no-AD engine by runtime across the full path-count sweep.  At
...
Rust local-volatility price requires further RNG-stream validation. Therefore,
the Rust rows support conclusions about Rayon throughput, but not independent
local-volatility pricing correctness.
...
cost per run shown on a logarithmic axis because the absolute costs are small
and differ by workload. In the stored priced comparison,
...
\begin{figure}[htbp]
    ...
    \caption{Runtime and log-scale estimated cost per run for best no-AD cloud configurations at \(M=100{,}000\). Lower-left points are preferable.}
...
be read as proof that SHA is generally reliable. In the measured headline
...
\subsection{Cross-Instance Profiler Robustness}
\label{subsec:cloud-profiler-robustness}

To test whether the headline result generalises beyond one instance slice, the
same full-grid, Pareto-recovery and regret calculations are applied to all six
priced instance-type slices stored in \code{sha\_cloud\_profiler\_v4}. Each
slice contains the same 16 candidate configurations and 48 full-grid cells.
Table~\ref{tab:profiler-cross-instance} reports the resulting post-hoc
robustness audit.

\input{evaluation/tables/profiler_cross_instance.tex}

This audit materially changes the interpretation of the profiler result. SHA
achieves perfect Pareto recovery and zero regret on the headline
\code{n2-standard-4} grid, but not on the other measured instance slices. In
five of the six slices SHA prunes too aggressively: it saves more full runs,
but misses at least one Pareto configuration and often fails to retain the
fastest \code{european/rust/none} configuration at \(M=100{,}000\). The old
profiler is less efficient, selecting 30 of 48 full cells in every slice, but
it recovers the Pareto frontier in all six slices.

The conclusion is therefore narrower and more useful: SHA is a promising
budget-reduction mechanism, but the current implementation is not yet robust
enough to replace the conservative profiler across instance types. A stronger
version should include uncertainty-aware promotion rules, repeated probes for
near-tie configurations, and a diversity or fastest-probe guard to avoid
discarding configurations whose scaling improves at larger \(M\).
...
and JAX agree within estimated Monte Carlo error, while Rust does not. Price
differences between engines may therefore reflect different random-number
generators, path-generation implementations, or, in the Rust case, a
backend-specific RNG-stream issue that requires further investigation.
...
\begin{itemize}
    ...
    fastest no-AD local-volatility runtime across the main \(M\)-sweep and
    the large stress test.  JAX/XLA and C++/OpenMP are close at larger
    local-volatility sizes, while NumPy is consistently slower. The Rust
    local-volatility price is not used as correctness evidence because it lies
    outside the estimated Monte Carlo uncertainty band.
    ...
    benchmark cells on \code{n2-standard-4}, saving 50.0\% of full runs while
    preserving 100.0\% Pareto recovery and 0.0\% runtime and cost regret in
    that grid. A cross-instance audit shows that this is promising but not yet
    generally reliable: SHA achieves perfect recovery on only one of six
    measured instance slices, while the older conservative profiler recovers
    the Pareto frontier on all six.
\end{itemize}
...
reduce full cloud benchmarking work on a favourable measured grid, but also
shows that robust budget-aware selection requires more conservative or
uncertainty-aware promotion rules.
