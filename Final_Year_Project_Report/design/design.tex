...
Every benchmark run is fully described by a single, serialisable
\texttt{WorkloadConfig} object. The \texttt{config\_hash()} method serialises this
object into canonically sorted JSON and computes a truncated (8-character) SHA-256
digest. The hash is stable across machines and Python versions and is used to group
repeated runs of the same experiment for statistical comparison.

All sources of randomness are explicitly seeded. For a given engine on a given
machine, the same configuration produces bit-identical prices and paths. Different
engines use different RNG implementations (NumPy \texttt{default\_rng}, JAX
\texttt{PRNGKey}, C++ \texttt{mt19937\_64}, Rust \texttt{SmallRng}, CUDA \texttt{curand}),
so the same seed produces statistically equivalent but numerically distinct results
across engines --- this is an intentional design choice rather than a defect, and is
documented in the codebase. Cross-engine validation is therefore performed against
the Black--Scholes analytical solution rather than against bit-equal outputs.
...
\fbox{
\parbox{0.9\textwidth}{
\textbf{Diagram Placeholder: System Architecture}
...
implementation. It supports both workloads and \texttt{ad\_mode = "none"} only.
...
\texttt{jax.grad(price\_fn, argnums=(0,1,2,3,4,5))} call that differentiates
through the full $N$-step \texttt{scan}; forward-mode uses six \texttt{jax.jvp}
calls. JAX differentiates through \texttt{jax.lax.scan} efficiently via its
reverse-mode accumulation, avoiding the explosion in graph size that would occur
if the loop were unrolled.
...
\texttt{BenchmarkDB} is a thread-safe SQLite wrapper using a per-call connection
with \texttt{check\_same\_thread=False} and an explicit
\texttt{threading.Lock} that serialises writes. The database is opened in
Write-Ahead Logging (WAL) mode, which allows concurrent reads during writes ---
important because the Flask API serves read requests from the main thread while
the background worker writes results. The database file lives at
\texttt{results/benchmarks.db} and is created on first use.

\paragraph{Schema.}
The schema consists of a single \texttt{runs} table whose columns are defined
declaratively by the \texttt{\_FULL\_SCHEMA\_COLUMNS} list. Columns are grouped as
follows:

\begin{itemize}
    \item \textbf{Identity:} \texttt{id} (UUID, primary key), \texttt{experiment\_id},
          \texttt{experiment\_type}, \texttt{config\_hash}.
    \item \textbf{Status lifecycle:} \texttt{status} (\texttt{pending}/\texttt{running}/%
          \texttt{completed}/\texttt{failed}), \texttt{error\_message},
          \texttt{created\_at}, \texttt{started\_at}, \texttt{completed\_at}
          (all UTC ISO-8601).
    \item \textbf{Workload definition:} \texttt{workload\_type}, \texttt{M},
          \texttt{N}, \texttt{seed}, \texttt{config\_json}.
    \item \textbf{Engine / implementation:} \texttt{engine}, \texttt{language},
          \texttt{backend}.
    \item \textbf{Parallelism metadata:} \texttt{num\_threads},
          \texttt{vectorization\_flag}, \texttt{batch\_size}.
    \item \textbf{Runtime performance:} \texttt{mean\_runtime\_ms},
          \texttt{std\_runtime\_ms}, \texttt{min\_runtime\_ms},
          \texttt{max\_runtime\_ms}, \texttt{throughput\_paths\_per\_sec}.
    \item \textbf{AD metadata:} \texttt{ad\_mode}, \texttt{baseline\_mean\_ms},
          \texttt{ad\_overhead\_ratio}.
    \item \textbf{Numerical results:} \texttt{result\_value}, \texttt{greek\_delta},
          \texttt{greek\_vega}, \texttt{greek\_rho}.
    \item \textbf{Analytical reference:} \texttt{analytical\_price},
          \texttt{analytical\_delta}, \texttt{analytical\_vega},
          \texttt{analytical\_rho}.
    \item \textbf{Error metrics:} \texttt{abs\_price\_error}, \texttt{rel\_price\_error}
          and matching columns for Delta, Vega and Rho.
    \item \textbf{Resources:} \texttt{memory\_peak\_mb}.
    \item \textbf{Environment:} \texttt{cpu\_model}, \texttt{cpu\_architecture},
          \texttt{cpu\_count}, \texttt{memory\_gb}, \texttt{platform},
          \texttt{python\_version}, \texttt{numpy\_version},
          \texttt{jax\_version}, \texttt{blas\_backend}.
    \item \textbf{Cloud and profiling:} \texttt{cloud\_provider},
          \texttt{instance\_type}, \texttt{cloud\_region},
          \texttt{cloud\_zone}, \texttt{hourly\_price\_usd},
          \texttt{cost\_per\_run}, profiler-selection fields, and
...
The original proposal emphasised cloud resource selection as a practical
systems question rather than a purely local benchmark. The final design
therefore extends each benchmark row with cloud metadata whenever the run is
...
imposed by the architecture. Should the framework be scaled out to a
distributed Ray-based execution model (Stage~8 of the future-work plan),
PostgreSQL would be a natural replacement.
...
runs at different thread counts are treated as statistically distinct
configurations rather than as repeats.
