...
What are the trade-offs between programming languages, runtimes, and AD support? &
...
through JAX, which provides the AD-capable reference implementation. The
compiled C++ and Rust engines are therefore native no-AD baselines rather than
representatives of C++ or Rust AD libraries. &
\code{run_european_baseline.py};
...
How useful are JAX/XLA-style compiler and AD techniques? &
JAX is compared with NumPy and compiled CPU engines for no-AD runs, and with
matched no-AD baselines for forward- and reverse-mode AD overhead. &
\code{run_european_ad_analysis.py};
\code{run_localvol_suite.py} \\
\bottomrule
\end{tabularx}
